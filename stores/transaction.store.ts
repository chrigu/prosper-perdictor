...
export const useTransactionStore = defineStore('income', () => {
  ...
  const expenditures = ref<Transaction[][]>(Array.from({ length: 12 }, () => []))
  ...
  function addTransaction(transaction: Transaction, transactions: Ref<Transaction[][]>) {
    transactions.value[transaction.month].push(transaction)
  }

  function yearlyTotal(transactions: Ref<Transaction[][]>) {
    return transactions.value.flat().reduce((acc, transaction) => acc + transaction.amount, 0)
  }

  return { incomes, expenditures, addIncome, yearlyIncome, yearlyExpenditure, addExpenditure}
})